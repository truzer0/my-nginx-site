...
const DotMatrix: React.FC<DotMatrixProps> = ({
  ...
  const uniforms = useMemo(() => {
    const safeColors = colors?.length ? colors : [[0, 0, 0]];
    let colorsArray = [
      safeColors[0] || [0, 0, 0],
      safeColors[0] || [0, 0, 0],
      safeColors[0] || [0, 0, 0],
...
const ShaderMaterial = ({
  ...
  const material = useMemo(() => {
    ...
    for (const [name, uniform] of Object.entries(uniforms)) {
      const { value, type } = uniform as any;
      switch (type) {
        case "uniform1f":
          preparedUniforms[name] = { value: value as number };
          break;
        case "uniform3fv":
  ...
  useFrame(({ clock }) => {
    ...
    if (mat.uniforms?.u_time?.value !== undefined) {
